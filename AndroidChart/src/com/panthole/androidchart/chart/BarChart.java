...
public class BarChart extends XYChart {
	...
	private static final int SHAPE_WIDTH = 15;
	...
	protected ClickableArea[] clickableAreasForPoints(List<Float> points,
		...
		for (int i = 0; i < length; i += 2) {
			...
			if (mType == Type.STACKED) {
				...
				// Math.max(y, yAxisValue)), values.get(i), values.get(i + 1));
				int selectableBuffer = mRenderer.getSelectableBuffer();
				ret[i / 2] = new ClickableArea(new RectF(x - selectableBuffer,
						y - selectableBuffer, x + selectableBuffer, y
		...
		// selectableBuffer, points.get(i + 1)
		// - selectableBuffer, points.get(i) + selectableBuffer, points.get(i +
		// 1) + selectableBuffer),
	...
	private void drawBar(Canvas canvas, float xMin, float yMin, float xMax,
		...
		//System.out.println("xMin:" + xMin + ";yMin:" + yMin + "xMax:" + xMax
		//		+ "yMax:" + yMax);
		float temp;
		if (xMin > xMax) {
			temp = xMin;
		...
		if (renderer.isGradientEnabled()) {
			...
			// canvas.drawCircle(xMin, yMin, 25, paint);
			...
			case 0:
				// image =
				// BitmapFactory.decodeResource(GraphicalView.class.getResources(),
				// R.drawable.blue_selected);
				image = renderer.getPointBitmap();
				paint.setColor(Color.rgb(107, 210, 239));
				;
				break;
			...
			case 1:
				...
				break;
			case 2:
				// image = BitmapFactory.decodeStream(GraphicalView.class.g
				// .getResourceAsStream("image/2.png"));
				image = renderer.getPointBitmap();
				paint.setColor(Color.rgb(0, 175, 222));
				;
				break;
			case 3:
				// image =
				// BitmapFactory.decodeStream(GraphicalView.class.getResourceAsStream("image/3.png"));
				image = renderer.getPointBitmap();
				paint.setColor(Color.rgb(249, 209, 139));
				;
				break;
			case 4:
				// image =
				// BitmapFactory.decodeStream(GraphicalView.class.getResourceAsStream("image/4.png"));
				image = renderer.getPointBitmap();
				paint.setColor(Color.rgb(254, 174, 133));
				;
				break;
			case 5:
				// image =
				// BitmapFactory.decodeStream(GraphicalView.class.getResourceAsStream("image/5.png"));
				image = renderer.getPointBitmap();
				paint.setColor(Color.rgb(248, 105, 33));
				;
				break;

			case 6:
				// image =
				// BitmapFactory.decodeStream(GraphicalView.class.getResourceAsStream("image/11.png"));
				image = renderer.getPointBitmap();
				paint.setColor(Color.rgb(107, 210, 239));
				;
				break;
			case 7:
				// image =
				// BitmapFactory.decodeStream(GraphicalView.class.getResourceAsStream("image/21.png"));
				image = renderer.getPointBitmap();
				paint.setColor(Color.rgb(0, 175, 222));
				;
				break;
			case 8:
				// image =
				// BitmapFactory.decodeStream(GraphicalView.class.getResourceAsStream("image/31.png"));
				image = renderer.getPointBitmap();
				paint.setColor(Color.rgb(249, 209, 139));
				;
				break;
			case 9:
				// image =
				// BitmapFactory.decodeStream(GraphicalView.class.getResourceAsStream("image/41.png"));
				image = renderer.getPointBitmap();
				paint.setColor(Color.rgb(254, 174, 133));
				;
				break;
			...
			if (seriesIndex == 0) {
				...
				if (yMax - yMin > 0) {
					...
					canvas.drawBitmap(image,
							xMin + (xMax - xMin) / 2 - image.getWidth() / 2,
							yMin - image.getHeight() / 2, paint);

					canvas.drawBitmap(image,
							xMin + (xMax - xMin) / 2 - image.getWidth() / 2,
							yMax - image.getHeight() / 2, paint);
				}
			}
		}
	}
