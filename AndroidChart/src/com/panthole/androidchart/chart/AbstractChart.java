...
public abstract class AbstractChart implements Serializable
{
     ...
     */
    protected void drawBackground(DefaultRenderer renderer, Canvas canvas, int x, int y, int width, int height,
            Paint paint, boolean newColor, int color)
    {
     ...
     */
    protected int drawLegend(Canvas canvas, DefaultRenderer renderer, String[] titles, int left, int right, int y,
            int width, int height, int legendSize, Paint paint, boolean calculate)
    {
        float size = 32;
     ...
     * @param paint the paint to be used for drawing
     */
    protected void drawMinLine(Canvas canvas, String text, float x, float y, Paint paint, XYSeriesRenderer renderer)
    {
        if (text != null)
        {
            String[] lines = text.split("\n");
            ...
            for (int i = 0; i < lines.length; ++i)
            {
                ...
                paint.setColor(renderer.getGradientStartColor());
                paint.setStrokeWidth(renderer.getmBarWidth());
                y = y - 40;
                x = x - 3;
                float p = renderer.getmBarLength();
                // canvas.drawLine(x, y, x + p, y - p, paint); // 绘制直线
                // canvas.drawLine(x, y, x - p, y - p, paint); // 绘制直线
    ...
    protected void drawMaxLine(Canvas canvas, String text, float x, float y, Paint paint, XYSeriesRenderer renderer)
    {
        if (text != null)
        {
            String[] lines = text.split("\n");
    ...
    protected void drawLabel(Canvas canvas, String labelText, DefaultRenderer renderer, List<RectF> prevLabelsBounds,
            int centerX, int centerY, float shortRadius, float longRadius, float currentAngle, float angle, int left,
            int right, int color, Paint paint, boolean line, boolean display)
    {
    ...
    public SeriesSelection getSeriesAndPointForScreenCoordinate(Point screenPoint, Canvas canvas, Paint paint)
    {
