...
import com.panthole.androidchart.renderer.BasicStroke;
import com.panthole.androidchart.renderer.DefaultRenderer;
import com.panthole.androidchart.renderer.SimpleSeriesRenderer;
import com.panthole.androidchart.renderer.XYMultipleSeriesRenderer;
import com.panthole.androidchart.renderer.XYSeriesRenderer;
import com.panthole.androidchart.renderer.XYMultipleSeriesRenderer.Orientation;
...
public abstract class XYChart extends AbstractChart
{
    ...
    private Bitmap label_bg_red_Image, label_bg_blue_Image;
    ...
    protected void drawMax(Canvas canvas, String text, float x, float y, Paint paint, float extraAngle,
            XYSeriesRenderer renderer)
    {
        ...
        drawMaxLine(canvas, text, x, y, paint, renderer);

        // label_bg_blue_Image = BitmapFactory.decodeStream(GraphicalView.class
        // .getResourceAsStream("image/label_bg_blue.png"));
        // canvas.drawBitmap(label_bg_red_Image,x, label_y, null);
        if (angle != 0)
        {
            canvas.rotate(-angle, x, y);
            // canvas.scale(mScale, 1 / mScale);
    ...
    protected void drawMin(Canvas canvas, String text, float x, float y, Paint paint, float extraAngle,
            XYSeriesRenderer renderer)
    {
        float angle = -mRenderer.getOrientation().getAngle() + extraAngle;
        if (angle != 0)
        ...
        drawMinLine(canvas, text, x, y, paint, renderer);
        if (angle != 0)
        {
            canvas.rotate(-angle, x, y);
            // canvas.scale(mScale, 1 / mScale);
            ...
            int left, int top, int bottom, double xPixelsPerUnit, double minX, double maxX)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            if (showGridY)
            {
                paint.setColor(Color.rgb(203, 203, 2));
            ...
            for (int j = 0; j < length; j++)
            {
                ...
                if (or == Orientation.HORIZONTAL)
                {
                    if (showLabels && !textLabel)
                    {
                        ...
                        {
                            paint.setStrokeWidth(3);
                            canvas.drawLine(left + getLabelLinePos(axisAlign) - 6, yLabel, left, yLabel, paint);
                            drawText(canvas, getLabel(mRenderer.getLabelFormat(), label),
                                    left - mRenderer.getYLabelsPadding() - 15,
                                    yLabel - mRenderer.getYLabelsVerticalPadding() + 10, paint,
                    ...
                    {
                        // 自定义横向的格子线的颜色，样式
                        paint.setColor(Color.argb(125, 165, 165, 165));
        ...
        if (showLabels)
        {
            ...
            for (Double location : xTextLabelLocations)
            {
                if (minX <= location && location <= maxX)
                {
                    ...
                    canvas.drawCircle(xLabel, bottom, 5, paint);
                    paint.setStyle(Style.STROKE);
                    canvas.drawCircle(xLabel, bottom, 9, paint);
                    paint.setStyle(Style.FILL);
                    // 将横坐标左移mRenderer.getLabelsTextSize()*3
                    drawText(canvas, mRenderer.getXTextLabel(location), xLabel - mRenderer.getLabelsTextSize() * 3,
                            bottom + mRenderer.getLabelsTextSize() * 4 / 3, paint, mRenderer.getXLabelsAngle());
                    if (showCustomTextGridX)
                    {
                        // 自定义纵向的格子线的颜色，样式
                        paint.setAntiAlias(true);
                        paint.setColor(Color.argb(125, 203, 203, 203));
                        PathEffect effects = new DashPathEffect(new float[]{2, 8, 2, 8}, 1);
                        paint.setPathEffect(effects);
                        // 每条gridx线底部减少6个像素
                        canvas.drawLine(xLabel, bottom - 6, xLabel, top, paint);
                        paint.setPathEffect(null);
                    }
                }
            }
    ...
    public SeriesSelection getSeriesAndPointForScreenCoordinate(final Point screenPoint)
    {
        if (clickableAreas != null)
        	//将seriesIndex = clickableAreas.size() - 1改为seriesIndex = 0可以保证每次都点击到点
            for (int seriesIndex = 0; seriesIndex >= 0; seriesIndex--)
            {
